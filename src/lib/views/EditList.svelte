<script lang="ts">
	...
	let { is_new } = $props<{ is_new: boolean }>();
	...
	let id = $derived(all_recipes.length); // TODO: is this good enough?
...
<Toolbar
	title={is_new ? "New shopping list" : "Edit list"}
	back_path="/shopping-lists"
/>
		...
		<Stepper on_finish={save}>
			...
			{#snippet step1()}
				<div class="flex-col gap1">
					...
					<button onclick={try_clear} class="red">Reset</button>
